...
桌面端业务读写集中在 `src-tauri/src/storage/`。Rust 侧通过 `rusqlite` 独占 SQLite 连接，数据库文件是 `personal-notebook-v2.db`，大文件资产进入 app-managed `personal-notebook-assets-v2/`；这两个名称是历史兼容文件名。SQLite 只保存元数据和引用。前端通过 `src/lib/storageClient.ts` 调 typed Tauri commands，不直接发 SQL。
