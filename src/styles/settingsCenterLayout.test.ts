...
describe('settings center layout', () => {
  it('keeps a fixed left navigation and a flexible content column', () => {
    ...
    expect(cssRule('.settings-center-nav')).toContain('min-height: calc(100vh - 56px);')
    expect(cssRule('.settings-center-nav')).toContain('display: flex;')
    expect(cssRule('.settings-center-nav')).toContain('flex-direction: column;')
