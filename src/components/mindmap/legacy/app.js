(() => {
  const STORAGE_KEY = "flowboard.mindmap.v1";
  ...
  const TEXT_FONT_OPTIONS = [
    { label: "榛樿", value: "Inter, Segoe UI, sans-serif" },
    { label: "寰蒋闆呴粦", value: "Microsoft YaHei, PingFang SC, sans-serif" },
    { label: "瀹嬩綋", value: "SimSun, Songti SC, serif" },
  ...
  textToolbar.innerHTML = `
    <button class="text-toolbar-grip" type="button" data-text-move aria-label="绉诲姩鏂囨湰">鈰嫯</button>
    <select class="text-font-select" data-text-font aria-label="瀛椾綋">
      ${TEXT_FONT_OPTIONS.map((font) => `<option value="${font.value}">${font.label}</option>`).join("")}
    </select>
    <input class="text-size-input" data-text-size type="number" min="${TEXT_MIN_FONT_SIZE}" max="${TEXT_MAX_FONT_SIZE}" step="1" aria-label="瀛楀彿" />
    <input class="text-color-input" data-text-color type="color" aria-label="鏂囧瓧棰滆壊" />
    <button class="text-style-button" type="button" data-text-bold aria-label="鍔犵矖">B</button>
    <button class="text-style-button" type="button" data-text-italic aria-label="鏂滀綋">I</button>
    <button class="text-delete-button" type="button" data-text-delete aria-label="鍒犻櫎鏂囨湰">
  ...
  function normalizeImage(image) {
    ...
    return {
      ...
      src: image.src,
      name: typeof image.name === "string" ? image.name : "鍥剧墖",
  ...
  function renderControls() {
    ...
    if (historyStatus) {
      historyStatus.textContent = `鍙洖閫€ ${undoStack.length} 姝ワ紝鍙墠杩?${redoStack.length} 姝;
    }
    if (historyToggle) {
      historyToggle.disabled = !undoStack.length && !redoStack.length;
      historyToggle.title = `鍙洖閫€ ${undoStack.length} 姝ワ紝鍙墠杩?${redoStack.length} 姝;
    }
    if (historyMenu) {
  ...
  async function addImageFromFile(file, world = viewportCenterWorld()) {
    ...
    try {
      ...
      await addImageFromSource(src, world, file.name || "鍥剧墖");
      return true;
    } catch (error) {
      console.error(error);
      window.alert("鍥剧墖瀵煎叆澶辫触锛岃鎹竴寮犲浘鐗囧啀璇曘€?);
      return false;
    }
  }

  async function addImageFromSource(src, world, name = "鍥剧墖") {
  ...
  function getShapeTextEditor() {
    ...
    editor.setAttribute("aria-label", "鍥惧舰鏂囧瓧");
  ...
  function createTextElement(text) {
    ...
    element.dataset.id = text.id;
    element.innerHTML = `
      <div class="text-content" contenteditable="false" spellcheck="false" role="textbox" aria-label="鏂囨湰"></div>
  ...
  function createImageElement(image) {
    ...
    element.innerHTML = `
      ...
      <button class="image-delete" type="button" aria-label="鍒犻櫎鍥剧墖">
        <svg viewBox="0 0 24 24" aria-hidden="true">
          <path d="M4 7h16"></path>
  ...
  function renderConnectionHandles() {
    ...
    for (const [side, position] of Object.entries(positions)) {
      ...
      button.setAttribute("aria-label", "鍒涘缓杩炴帴");
      button.textContent = "+";
  ...
  function createNoteElement(note) {
    ...
    element.innerHTML = `
      <div class="note-grip">
        ...
        <button class="note-delete" type="button" aria-label="鍒犻櫎渚跨">
          <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M6 6l12 12"></path><path d="M18 6L6 18"></path></svg>
        </button>
      </div>
      <textarea class="note-body" spellcheck="false" placeholder="鍐欑偣浠€涔?></textarea>
  ...
  async function drawExportImage(image) {
    try {
      ...
      try {
        ...
        ctx.textAlign = "center";
        ctx.textBaseline = "middle";
        ctx.fillText("鍥剧墖鏃犳硶瀵煎嚭", image.x + image.w / 2, image.y + image.h / 2);
      } finally {
        ctx.restore();
      }
    }
  ...
  function clearBoard() {
    ...
    if (!window.confirm("娓呯┖褰撳墠鐧芥澘锛?)) return;
    pushHistory();
    state.shapes = [];
    state.strokes = [];
    state.connections = [];
