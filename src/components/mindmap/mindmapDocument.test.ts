...
import { buildLegacyMindmapSrcDoc } from './mindmapDocument'
import { createEmptyMindmapSnapshot } from './mindmapModel'

describe('legacyMindmapDocument', () => {
  it('includes pointer-release completion for connection drafts', () => {
    const srcDoc = buildLegacyMindmapSrcDoc('mindmap-1', createEmptyMindmapSnapshot(), '2026-06-19T00:00:00.000Z')
  ...
  it('updates history controls with live counts and disabled states', () => {
    ...
    expect(srcDoc).toContain('historyStatus.textContent = ')
  })

  it('re-renders note connection handles immediately when a note body is clicked', () => {
    const srcDoc = buildLegacyMindmapSrcDoc(
      'mindmap-1',
  ...
  it('preserves mindmap selection when the host pushes a replacement snapshot', () => {
    const srcDoc = buildLegacyMindmapSrcDoc(
      'mindmap-1',
      createEmptyMindmapSnapshot(),
      '2026-06-19T00:00:00.000Z',
