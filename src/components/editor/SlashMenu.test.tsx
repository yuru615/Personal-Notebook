...
describe('SlashMenu', () => {
  it('renders grouped sections for core block types', () => {
    ...
    expect(
      screen.getByText('页面与数据', { selector: '.slash-menu-section-label' }),
    ).toBeInTheDocument()
  })
  ...
  it('shows the mindmap option', () => {
    render(<SlashMenu query="/鎬濈淮瀵煎浘" onPick={vi.fn()} />)

    expect(screen.getByRole('button', { name: '鎬濈淮瀵煎浘' })).toBeInTheDocument()
    expect(screen.getByText('鎻掑叆涓€涓彲鐐瑰嚮杩涘叆鐨勬€濈淮瀵煎浘鍏ュ彛')).toBeInTheDocument()
  })
})
