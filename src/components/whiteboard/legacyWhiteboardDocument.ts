...
const hostBridgeSnippet = `${storageKeySnippet}
  ...
  function flowboardShadowStorageKey() {
    ...
    return payload ? STORAGE_KEY + "." + payload.boardId : STORAGE_KEY;
  }
  ...
  window.addEventListener("message", (event) => {
    ...
    if (!payload || !data || data.source !== "flowboard-host" || data.boardId !== payload.boardId) return;
    if (data.type !== "flowboard-replace") return;
...
function patchLegacyAppScript() {
  // ponytail: patch the original whiteboard in place instead of rewriting 4k+ lines into React.
...
export function buildLegacyWhiteboardSrcDoc(
  boardId: string,
