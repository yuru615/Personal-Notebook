...
export function summarizeWhiteboardSnapshot(snapshot: WhiteboardSnapshot): WhiteboardSummary {
  const elementCount =
...
export function buildWhiteboardPreviewSvgDataUrl(snapshot: unknown): string | null {
  if (!isWhiteboardSnapshot(snapshot)) {
    return null
  }

  const normalizedSnapshot = normalizeWhiteboardSnapshot(snapshot)

  const summary = summarizeWhiteboardSnapshot(normalizedSnapshot)
  if (summary.isEmpty) {
    return null
  }
...
function buildConnectableCenterMap(snapshot: WhiteboardSnapshot) {
...
function renderStroke(stroke: WhiteboardStroke, scale: number, offsetX: number, offsetY: number) {
...
function renderNote(note: WhiteboardNote, scale: number, offsetX: number, offsetY: number) {
...
function renderShape(shape: WhiteboardShape, scale: number, offsetX: number, offsetY: number) {
...
function renderImage(image: WhiteboardImage, scale: number, offsetX: number, offsetY: number) {
...
function renderText(text: WhiteboardText, scale: number, offsetX: number, offsetY: number) {
...
function getSnapshotBounds(snapshot: WhiteboardSnapshot): Bounds {
  const bounds: Bounds[] = [
...
function strokeBounds(stroke: WhiteboardStroke): Bounds | null {
  if (stroke.points.length === 0) {
    return null
  }
