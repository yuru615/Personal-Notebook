...
## 2026-07-16 高中语文教师工作台页面包设计

简要描述：

确定一套可重复导入的高中语文教师个人备课与知识沉淀工作台模板，使用高一上册第七单元作为完整示例。

详细描述：

- 模板采用单根页面树页面包，覆盖教师工作台、教学规划、五篇课文备课、教学执行、资源与知识库、作业与匿名学情、课后与单元复盘。
- 设计三张数据表、两块白板、两张思维导图、两组同步块，以及图片、文件、页面提及和双向链接示例。
- 页面包导入后追加到现有工作区并自动重建对象 ID；不修改欢迎工作区，不建设模板中心，不携带真实学生数据或受版权保护的教材与课件。
- 规格明确了可维护模板构建器、可重复生成脚本、结构与引用校验、真实导入及重复导入验收。
- 已补充逐步实施计划，按页面内容、结构化资源、页面包生成与真实桌面验收四个阶段执行，并坚持测试先行和最小文件范围。

验证情况：

- 已完成设计自检，确认无占位内容、内部引用边界矛盾或未定义的交付范围；实现与运行验证将在规格复核通过后进行。
