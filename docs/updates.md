...
- 恢复会先在同一自动备份目录生成带 `protection-` 前缀的完整保护归档，保护归档不会参与自动保留清理。保护归档失败时不会覆盖当前工作区，并将明确警告返回给界面；损坏或不可读的自动备份也不会替换当前内容。
- 新增 `begin_auto_backup_session`、`run_auto_backup` 与 `restore_latest_auto_backup` 桌面命令，并在真正退出前持久化正常结束状态。

验证情况：

- 按 red → green 新增异常退出/正常退出、禁用与间隔调度、仅应用设置变更、工作区替换后状态保留、成功恢复、损坏归档与保护备份失败不覆盖工作区的 Rust 回归测试。
- 自动备份定向测试 21 项通过；两项名称未包含筛选词的“仅应用设置”与“保护归档失败”测试已单独通过；完整 Rust 测试 209 项通过、0 失败。
