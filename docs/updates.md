...
- 会话替换在用户行锁内复核密码哈希、停用状态和邮箱验证状态，避免密码重置或账号停用与登录并发时使用旧认证结果创建会话。
- 被替换的客户端会显示“账号已在其他设备登录”，保存待写入的本地内容后清除远程凭据，但不会删除本地知识库。
- 已登录客户端每 30 秒及窗口重新获得焦点时校验会话；断网或服务端暂时不可用时继续保留现有的 24 小时离线使用能力。

验证情况：
- 服务端 `go test ./...` 与 `go vet ./...` 通过；客户端账号门禁相关 12 条前端测试通过，`npm run lint` 无错误（保留 5 条既有 Hooks warning），`npm run build` 通过。
- Rust 完整测试 159 条通过，`cargo check --locked` 通过，账号模块单文件 `rustfmt --check` 通过。
- 前端全量测试共 813 条通过、1 条失败；失败为既有 `src-tauri/tauriConfig.test.ts` 仍断言版本 `0.1.0`，而当前项目版本已是 `0.1.1`，与本次会话改动无关。
- 已在隔离 PostgreSQL 17 中验证迁移前多会话清理、过期会话撤销、单会话唯一约束、并发登录串行化，以及密码重置后拒绝旧登录结果创建会话。
...
知栖桌面客户端不再显示 WebView 的浏览器原生右键菜单，降低网页感和误触调试类菜单项的风险。

详细描述：
- 在 React 应用入口以捕获阶段统一拦截 `contextmenu` 事件，覆盖登录、工作区、弹窗和动态挂载界面。
- 思维导图运行在独立 iframe 文档中，因此在宿主增强脚本内同步禁用原生右键菜单。
- 此改动不影响键盘复制、剪切和粘贴快捷键；右键菜单禁用属于桌面体验收敛，不替代 Tauri 权限和命令边界等安全措施。

验证情况：
- 已通过完整前端测试，共 97 个测试文件、810 条测试；`npm run build` 通过。
- `npm run lint` 通过且无错误，保留 5 条与本次改动无关的既有 Hooks warning。

## 2026-07-16 客户端自动更新与版本发布

提交：未提交

简要描述：

新增 Windows x64 与 macOS Apple Silicon 客户端版本发布和应用内自动更新能力，安装包通过私有阿里云 OSS 分发。

详细描述：

- 客户端版本统一为 `0.1.0`，接入 Tauri updater/process 插件，启动时检查更新，并在设置中心提供手动检查入口。
- 可选更新支持稍后处理；低于服务端最低支持版本时显示更新门禁。检查或下载故障允许本次离线进入，避免阻断本地知识库。
- 更新包下载后先完成 Tauri 签名校验，安装前刷新所有待保存内容，保存失败时取消退出和安装。
- 账号服务管理后台新增版本草稿、三项平台产物直传、发布和撤回；服务端使用私有 OSS 短时签名 URL，数据库只保存版本和文件元数据。
- Windows 发布 NSIS 产物，macOS 只支持 Apple Silicon 的 updater 归档与 DMG，不构建或发布 Intel 版本；正式发布仍需 Apple 签名/公证和 Windows Authenticode。

验证情况：

- 已通过客户端 96 个测试文件、807 条前端测试，前端生产构建和 lint（0 error，保留 5 条既有 Hooks warning）。
- 已通过 Rust 158 条测试、`cargo check --locked`，以及服务端 Go 全量测试和 `go vet ./...`。
- 管理后台 3 个测试文件、13 条测试和生产构建通过；仓库既有 lint 脚本因未安装 ESLint 且没有 ESLint 配置而无法执行。
- 已在隔离 Docker Compose 栈应用数据库迁移 4，确认 API readiness，并通过真实管理后台创建 `0.1.0` 草稿。
- 已在隔离 PostgreSQL 中验证三项产物完整发布、Tauri manifest、同版本 `204`、首装 `307`、审计记录及撤回后停止更新。
- 已使用浏览器检查桌面和 390×844 窄屏版本发布页面，页面无重叠，控制台无 warning/error。

## 2026-07-15 Tauri 启动告警清理

提交：未提交

简要描述：

清理桌面开发启动时输出的 Rust 无用代码告警，使启动日志只保留实际异常信息。

详细描述：

- 测试专用的 MCP 内容归一化和旧版块追加辅助仅在 Rust 测试构建中编译。
- MCP 写入结果保留给当前调用方使用，但不再从存储模块暴露未使用的公开导出。

验证情况：

- 已通过完整 Rust 测试：157 passed、0 failed；`cargo check --locked` 无 warning。
- 已通过 macOS `npm run tauri:dev` 实际启动验证，Rust 编译与应用启动日志不再输出无用代码 warning。
- 浏览器静态门禁页正常渲染，控制台无 `warn` 或 `error`。
...
桌面端消息入口现已接入账号服务发布的公告，登录用户可在应用内浏览公告列表与富文本详情。

详细描述：

- 侧边栏铃铛从禁用占位改为消息中心入口，紧凑与经典两种侧边栏均可访问。
- 公告列表支持分页、刷新、空状态、离线与错误重试，详情显示发布时间和服务端清理后的富文本正文。
- 公告请求继续由 Tauri Rust 使用系统凭据中的 Bearer Token 发出，React 不接触 Token；会话失效会复用现有保存待写内容并锁定应用的流程。
- 客户端会对公告 HTML 再执行一次标签、属性和 HTTP(S) 外链白名单清理，外链通过原生安全打开边界处理。

验证情况：

- 已通过完整前端测试：95 个测试文件、802 条测试；`npm run lint` 无错误（5 条既有 Hooks 警告），`npm run build` 通过。
- 已通过完整 Rust 测试：157 passed、0 failed，并通过 `cargo check`；macOS `npm run tauri:dev` 完成编译并成功启动当前调试二进制。
- 已通过服务端 `go test ./...`、`go vet ./...`，以及管理后台 12 条测试和生产构建。
- 浏览器插件拦截本地回环地址，无法作为本次截图通道；消息首条自动展示、切换、离线错误和会话失效锁定均由组件测试覆盖。
