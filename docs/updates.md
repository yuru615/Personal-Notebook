...
- 自动备份只识别自身严格命名的 `.zhiqi` 文件，按可排序时间戳从新到旧保留指定份数；下次自动备份会回收严格匹配的遗留临时文件，手动备份和其他临时文件不会被清理。

验证情况：

- 已按 red → green 新增 15 份归档保留 14 份、每份含媒体附件可由既有工作区导入恢复、失败不清理已发布归档、同名目标不覆盖、遗留自动临时文件回收且手动临时文件保留的 Rust 回归测试。
