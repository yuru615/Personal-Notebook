...
- 新归档先写入同目录临时文件，写入完成后再以原子重命名发布；导出、写入或发布失败时不会触发旧归档清理。
- 自动备份只识别自身严格命名的 `.zhiqi` 文件，按可排序时间戳从新到旧保留指定份数；临时文件、手动备份和其他文件不会被清理。

验证情况：

- 已按 red → green 新增 15 份归档保留 14 份、每份含媒体附件可由既有工作区导入恢复、失败不清理已发布归档、临时和非自动文件不清理的 Rust 回归测试。
- 自动备份定向 Rust 测试 10 项通过；完整 Rust 测试 195 项通过。
