...
- 按 red → green 新增异常退出/正常退出、禁用与间隔调度、仅应用设置变更、工作区替换后状态保留、成功恢复、损坏归档、保护备份失败仍恢复、导入阶段失败不覆盖工作区，以及托盘直接退出标记 clean 的 Rust 回归测试。
- 自动备份定向测试 23 项通过；完整 Rust 测试 211 项通过、0 失败。
