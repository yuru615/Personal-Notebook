...
- 模板采用单根页面树，共包含 36 个页面、3 张数据表（分别为 22、16、8 条记录）、2 块白板、2 张思维导图、2 组同步块和 2 个内嵌资产，并保留图片、文件、页面提及和双向链接示例。
...
- 模板 validator 会检查结构、引用和资源契约，生成脚本可确定性重建页面包；真实 Rust 双次导入验证会生成两棵 ID 独立的页面树。
...
- 最终页面包大小为 34,581 bytes，SHA-256 为 `71C04EADB838BE9B831DF7599406957B5B49926CC15D3A532F298DFFC5F7829E`。
