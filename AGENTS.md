...
这是一个本地优先的类 Notion 个人知识库 Web 应用。核心数据保存在浏览器 IndexedDB 中，前端直接提供页面编辑、结构化内容、白板、数据表、思维导图和导入导出能力。
...
- 数据表/公式/导出辅助：`expr-eval`、`jszip`、`idb`。
...
│   ├── lib/                      # IndexedDB/Dexie 和 WorkspaceRepository
│   ├── store/                    # 工作区状态和业务操作
...
- 工作区数据变更应通过 `src/store/createWorkspaceStore.ts` 暴露的操作完成，并经 `WorkspaceRepository` 持久化；不要在组件里直接写 Dexie。
- IndexedDB schema 变更集中在 `src/lib/db.ts`，新增表或索引时要考虑旧版本迁移和导入 JSON 的兼容。
...
- 测试文件与被测文件就近放置，命名为 `*.test.ts` 或 `*.test.tsx`；涉及 IndexedDB 的测试依赖 `src/test/setup.ts` 中的 `fake-indexeddb`。
...
- 修改路由或导航时，同时检查页面树、面包屑、最近打开页面和缺失资源恢复逻辑。
- 修改编辑器交互时，同时检查键盘输入、组合输入、富文本、浮层关闭、拖拽、撤销重做。
- 修改样式时至少检查桌面和窄屏布局，避免按钮文字溢出、浮层遮挡、嵌套卡片和不必要的单色调视觉。
- 完成代码改动后按风险运行验证：小改动至少运行相关测试；共享逻辑或数据模型改动运行 `npm test`；发布前运行 `npm run lint` 和 `npm run build`。
